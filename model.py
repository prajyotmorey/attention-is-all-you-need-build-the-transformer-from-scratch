...
def build_id_to_token_vocab(token_to_id):
    ...
    id_to_token_dict = {token_id : token for token, token_id in token_to_id.items()}
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    for token in sentence.split():
        if token in token_to_id:
            encode_sentence.append(token_to_id[token])
        else:
...
def decode_ids_to_tokens(ids, id_to_token):
    ...
    if ids is None:
        ...
        for i in ids:
            decoded_tokens.append(id_to_token[i])
...
def scale_embeddings_by_sqrt_d_model(embeddings, d_model):
    ...
    scaled_embedding_sqrt = embeddings * math.sqrt(d_model)
    
    return scaled_embedding_sqrt
